...
We do *not* target a collateralization ratio. We run a parity machine (PSM) and use **convert-on-deposit** via the AutoConverter to keep reserves stable-denominated. See: [PSM vs CDP](PSM_VS_CDP.md).
...
On-chain [Parameter Registry](PARAMETER_REGISTRY.md). Writes only through DAO/Timelock. Guardian can *pause only* and auto-expires (sunset).
...
No consensus changes. We use an indexer finality watermark and adapt to Kaspa throughput as it evolves. See: [Kaspa Integration Notes](KASPA_INTEGRATION_NOTES.md).
