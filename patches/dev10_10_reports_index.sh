...
echo "== DEV-10 10: create Reports & Status index =="

REPORTS_DIR="docs/reports"
INDEX_FILE="${REPORTS_DIR}/REPORTS_INDEX.md"
...
# Wire a small link from docs/index.md if not already present
if [ -f "$DOCS_INDEX" ]; then
  if grep -q "Reports & Status Index" "$DOCS_INDEX"; then
    echo "Reports & Status Index section already present in docs/index.md, nothing to do."
  else
    cat <<'EOD' >> "$DOCS_INDEX"
...
EOD
  ...
  echo "docs/index.md not found, skipped linking."
fi

# Log entry
LOG_FILE="logs/project.log"
timestamp="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "[DEV-10 10] ${timestamp} Created REPORTS_INDEX.md and linked it from docs/index.md" >> "$LOG_FILE"
