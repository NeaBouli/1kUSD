...
echo "== DEV-11 02: add StrategyEnforcement Phase A advanced spec (docs-only) =="

# 1) Ensure directories exist
mkdir -p docs/architecture
mkdir -p docs/dev

# 2) Write Phase A spec document
cat <<'EOD' > docs/architecture/buybackvault_strategy_phaseA_advanced.md
...
# 3) Cross-link from DEV11 plan (if present)
DEV11_PLAN="docs/dev/DEV11_BuybackVault_EconomicAdvanced_Plan_r1.md"
if [ -f "$DEV11_PLAN" ]; then
  if ! grep -q "StrategyEnforcement Phase A" "$DEV11_PLAN"; then
    cat <<'EOP' >> "$DEV11_PLAN"
...
EOP
    ...
    echo "DEV11 plan already references Phase A spec"
  fi
else
  echo "DEV11 plan not found, skipping cross-link."
fi

# 4) Link from docs/INDEX.md under Economic Layer – Future / Advanced (DEV-11)

INDEX_FILE="docs/INDEX.md"
if [ -f "$INDEX_FILE" ]; then
  if ! grep -q "buybackvault_strategy_phaseA_advanced" "$INDEX_FILE"; then
    cat <<'EOT' >> "$INDEX_FILE"
...
EOT
    ...
    echo "docs/INDEX.md already links buybackvault_strategy_phaseA_advanced"
  fi
else
...
echo "[DEV-11 02] ${timestamp} Added StrategyEnforcement Phase A advanced spec (docs-only, no contract changes)" >> "$LOG_FILE"

echo "== DEV-11 02: StrategyEnforcement Phase A spec created (docs-only) =="
