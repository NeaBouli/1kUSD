...
echo "== DEV-11 03: add Telemetry & Events outline (docs-only) =="

# 1) Ensure dev directory exists
mkdir -p docs/dev

# 2) Write outline document
cat <<'EOD' > docs/dev/DEV11_Telemetry_Events_Outline_r1.md
...
# 3) Link from docs/INDEX.md (DEV-11 section)

INDEX_FILE="docs/INDEX.md"
if [ -f "$INDEX_FILE" ]; then
  if ! grep -q "DEV11_Telemetry_Events_Outline_r1" "$INDEX_FILE"; then
    cat <<'EOT' >> "$INDEX_FILE"
...
EOT
    ...
    echo "docs/INDEX.md already links DEV11_Telemetry_Events_Outline_r1"
  fi
else
...
echo "[DEV-11 03] ${timestamp} Added DEV11_Telemetry_Events_Outline_r1 (docs-only, no contract changes)" >> "$LOG_FILE"

echo "== DEV-11 03: Telemetry & Events outline created (docs-only) =="
