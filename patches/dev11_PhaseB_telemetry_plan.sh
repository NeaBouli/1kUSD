...
# DEV-11 Phase B – Telemetry & Test Plan for Buyback Safety

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR/.."

PLAN_DOC="docs/dev/DEV11_PhaseB_Telemetry_TestPlan_r1.md"
BACKLOG_DOC="docs/dev/DEV11_Implementation_Backlog_SolidityTrack_r1.md"

echo "== DEV-11 Phase B: writing telemetry & test plan =="

# 1) Phase-B Plan-Dokument schreiben/überschreiben
cat <<'EOF_MD' > "$PLAN_DOC"
...
echo "Wrote Phase B telemetry/test plan to $PLAN_DOC"

# 2) Backlog-Dokument um Phase B Abschnitt erweitern (am Ende anhängen)
cat <<'EOF_BL' >> "$BACKLOG_DOC"
...
echo "Appended Phase B planning section to $BACKLOG_DOC"

# 3) MkDocs Build zur Sicherheit
mkdocs build

# 4) DEV-11 Phase B Log-Eintrag
echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') DEV-11 PhaseB telemetry_plan: add Phase B telemetry/test plan and backlog entry" >> logs/project.log

echo "== DEV-11 Phase B telemetry_plan done =="
