#!/usr/bin/env bash
set -euo pipefail
echo "== DEV-41-T1: create OracleRegression_Watcher.t.sol =="

cat <<'EOT' > foundry/test/oracle/OracleRegression_Watcher.t.sol
...
mkdir -p logs
printf "%s DEV-41-T1: added OracleRegression_Watcher.t.sol (status propagation)\n" "$(date -u +'%Y-%m-%dT%H:%M:%SZ')" >> logs/project.log
echo "✅ OracleRegression_Watcher.t.sol created successfully."
