#!/usr/bin/env bash
set -euo pipefail
echo "== DEV-42 Patch 1: OracleAggregator.sol =="

cat > contracts/core/OracleAggregator.sol <<"EOL"
...
contract OracleAggregator is IOracleAggregator {
    ...
    function setPriceMock(address asset, int256 price, uint8 decimals, bool healthy)
        ...
        notPaused
    {
        _mockPrice[asset] = Price(price, decimals, healthy, block.timestamp);
        emit OracleUpdated(asset, price, decimals, healthy);
    }
}
EOL
echo "✅ OracleAggregator.sol updated."
