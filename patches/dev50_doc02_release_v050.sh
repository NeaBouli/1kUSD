...
FILE="docs/releases/v0.50.0_economic-layer.md"

echo "== DEV50 DOC02: write v0.50.0 Economic Layer release notes =="

mkdir -p "$(dirname "$FILE")"

cat <<'EOL' > "$FILE"
# v0.50.0 – Economic Layer
...
- \`swapTo1kUSD\`:
  - Pulls collateral from user → \`CollateralVault\`
...
- Behaviour:
  - If no registry or no entry → safe fallback to **18 decimals**
...
## 🇩🇪 Kurzfassung (für internes Protokoll)
...
- Der Oracle-Stack besitzt **Health-Gates für alte Preise und zu große Preis-Sprünge**, die in Guardian/Safety einfließen.
- Docs und Logs sind aktualisiert, sodass DEV-43–52 als **sauberes ökonomisches Paket** nachvollziehbar sind.
EOL

echo "✓ v0.50.0 Economic Layer release notes written to $FILE"
