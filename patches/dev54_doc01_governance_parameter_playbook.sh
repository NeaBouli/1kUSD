...
echo "== DEV54 DOC01: write Governance Parameter Playbook (DE) =="
...
mkdir -p "$(dirname "$LOG")"
cat <<EOL >> "$LOG"
[DEV-54] $(date -u +"%Y-%m-%dT%H:%M:%SZ") Docs: added governance/parameter_playbook.md (DE) describing PSM fees/spreads/decimals, PSMLimits caps, oracle health thresholds and admin/registry roles for DAO/Risk Council.
EOL

echo "✓ Governance Parameter Playbook written to $FILE and log updated."
