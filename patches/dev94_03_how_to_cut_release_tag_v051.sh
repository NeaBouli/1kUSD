...
echo "== DEV-94 03: add maintainer guide 'How to cut a release tag (v0.51.x)' =="

# 1) Write maintainer guide
mkdir -p docs/dev

cat <<'EOD' > docs/dev/DEV94_How_to_cut_a_release_tag_v051.md
...
DEV94_How_to_cut_a_release_tag_v051 – Step-by-step guide for maintainers cutting v0.51.x tags.
EOL
else
echo "DEV94_How_to_cut_a_release_tag_v051 already linked in docs/INDEX.md"
fi
else
echo "docs/INDEX.md not found, skipping index link."
fi

3) Add 'Further reading' hint to DEV94_ReleaseFlow_Plan_r2.md (if not present)
PLAN_FILE="docs/dev/DEV94_ReleaseFlow_Plan_r2.md"
if [ -f "$PLAN_FILE" ]; then
if ! grep -qi "Further reading" "$PLAN_FILE"; then
cat <<'EOM' >> "$PLAN_FILE"

Further reading
For a practical step-by-step maintainer guide on how to cut a v0.51.x
release tag, see
DEV94_How_to_cut_a_release_tag_v051.
EOM
else
...
echo "[DEV-94 03] ${timestamp} Added DEV94_How_to_cut_a_release_tag_v051 guide and linked it from docs index and DEV94 plan" >> "$LOG_FILE"
