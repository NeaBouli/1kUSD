...
# 1) DEV-9 Architecture Sync Report schreiben
cat <<'DOC' > docs/reports/DEV9_ArchitectureSync_Oracle_Safety_r1.md
...
# 2) Report in den REPORTS_INDEX einhängen
python - << 'PY'
from pathlib import Path

idx = Path("docs/reports/REPORTS_INDEX.md")
text = idx.read_text()

line = "- [DEV9_ArchitectureSync_Oracle_Safety_r1](DEV9_ArchitectureSync_Oracle_Safety_r1.md) – DEV-9 architecture sync on oracles & safety (Dec 2025)\n"

if "DEV9_ArchitectureSync_Oracle_Safety_r1" not in text:
    text = text.rstrip() + "\n" + line
    idx.write_text(text)
PY

# 3) Log-Eintrag
echo "[DEV-9 40] $(date -u +"%Y-%m-%dT%H:%M:%SZ") Added DEV9_ArchitectureSync_Oracle_Safety_r1 architecture sync report (oracles & safety clarifications)" >> logs/project.log

echo "== DEV-9 40: architecture sync report written =="
